...
impl<'a, T: NetworkBytesMut + Debug> EthIPv4ArpFrame<'a, T> {
    #[tracing::instrument(level = "trace", ret(skip), skip(buf,htype,ptype,hlen,plen,operation,sha,spa,tha,tpa))]
    ...
    #[tracing::instrument(level = "trace", ret(skip), skip(buf,sha,spa,tha,tpa))]

...
impl<'a, T: NetworkBytesMut + Debug> EthernetFrame<'a, T> {
    #[tracing::instrument(level = "trace", ret(skip), skip(buf,dst_mac,src_mac,ethertype))]
...
impl<'a, T: NetworkBytes + Debug> Incomplete<EthernetFrame<'a, T>> {
    #[tracing::instrument(level = "trace", ret(skip), skip(self,payload_len))]
...
mod kani_proofs {
    ...
    mod stubs {
        ...
        #[tracing::instrument(level = "trace", ret(skip), skip(input))]

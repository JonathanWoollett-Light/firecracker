...
impl TcpIPv4Handler {
    ...
    pub fn receive_packet<T: std::fmt::Debug + NetworkBytes, F: FnOnce(Request) -> Response>(
        &mut self,
        packet: &IPv4Packet<T>,
        callback: F,
    ...
    fn enqueue_rst<T: std::fmt::Debug + NetworkBytes>(
        &mut self,
        tuple: ConnectionTuple,
        s: &TcpSegment<T>,
    ) {
...
mod tests {
    ...
    fn inner_tcp_mut<'a, T: std::fmt::Debug + NetworkBytesMut>(

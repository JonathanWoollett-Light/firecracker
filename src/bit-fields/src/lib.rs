...
impl<T, const START: u8, const END: u8> BitRange<T, START, END> {
    ...
    // This only exists becuase `default` is not const, which is only the case since const traits
    // are not stabilized yet.
    /// ```
    /// let _bit_rnage = bit_fields::BitRange::<u8,2,4>::new();
    /// ```
    #[must_use]
    #[inline]
...
impl fmt::Display for TryFromFlagSetError {
    /// ```
    /// assert_eq!(bit_fields::TryFromFlagSetError.to_string(),"Feature flag given in set which is not defined in bit field");
    /// ```
    #[inline]
...
impl fmt::Display for CheckedAssignError {
    /// ```
    /// assert_eq!(bit_fields::CheckedAssignError.to_string(),"Given value is greater than maximum storable value in bit range");
    /// ```
    #[inline]
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
...
macro_rules! bit_range_impl {
    ($x:ty,$mask:ident,$max:ident) => {
        impl<const START: u8, const END: u8> BitRange<$x, START, END> {
            ...
            /// Returns a mutable pointer to undelying data.
            ...
            // `x <= Self::MAX` gurantees `x << START` is safe and `x <= Self::MAX - cur` gurantees
            ...
            // `x <= Self::MAX` gurantees `x << START` is safe and `x <= cur` gurantees
            // `self.data_mut() -= shift` is safe.
            #[allow(clippy::integer_arithmetic)]
            #[inline]
...
macro_rules! bit_range_from_impl {
    ($x:ty,$(#[$attr:meta])*) => {
        ...
        // compile-time and would be immedately obvious. Thus it is safe to use arithmetic here.
        #[allow(clippy::integer_arithmetic)]
        $(#[$attr])*
...
mod tests {
    ...
    #[test]
    fn display() {
        const FIELD_U16: BitFieldu16 = BitFieldu16::new(0b1010_1110_0101_0111);
        const FIELD_U8: BitFieldu8 = BitFieldu8::new(0b1010_1110);
        #[rustfmt::skip]
    ...
    #[test]
    fn size() {
        static_assertions::assert_eq_size!(BitFieldu128, u128);
        static_assertions::assert_eq_size!(BitFieldu64, u64);
        static_assertions::assert_eq_size!(BitFieldu32, u32);
        static_assertions::assert_eq_size!(BitFieldu16, u16);
        static_assertions::assert_eq_size!(BitFieldu8, u8);
    }
    // Testing masks work
    #[test]
    fn mask() {
        // These top level checks exists to communicate to code coverage that this function is covered
        ...
        // u128
        const_assert_eq!(mask_u128::<0, 128>(), 0xffff_ffff_ffff_ffff_ffff_ffff_ffff_ffff);
        const_assert_eq!(mask_u128::<0, 64>(), 0x0000_0000_0000_0000_ffff_ffff_ffff_ffff);
        const_assert_eq!(mask_u128::<64, 128>(), 0xffff_ffff_ffff_ffff_0000_0000_0000_0000);
        // u64
        const_assert_eq!(mask_u64::<0, 64>(), 0xffff_ffff_ffff_ffff);
        const_assert_eq!(mask_u64::<0, 32>(), 0x0000_0000_ffff_ffff);
        const_assert_eq!(mask_u64::<32, 64>(), 0xffff_ffff_0000_0000);
        const_assert_eq!(mask_u64::<4, 60>(), 0x0fff_ffff_ffff_fff0);
        const_assert_eq!(mask_u64::<8, 56>(), 0x00ff_ffff_ffff_ff00);
        const_assert_eq!(mask_u64::<12, 52>(), 0x000f_ffff_ffff_f000);
        const_assert_eq!(mask_u64::<16, 48>(), 0x0000_ffff_ffff_0000);
        const_assert_eq!(mask_u64::<20, 44>(), 0x0000_0fff_fff0_0000);
        const_assert_eq!(mask_u64::<24, 40>(), 0x0000_00ff_ff00_0000);
        const_assert_eq!(mask_u64::<28, 36>(), 0x0000_000f_f000_0000);
        const_assert_eq!(
            mask_u64::<30, 34>(),
        ...
        // u32
        const_assert_eq!(mask_u32::<0, 32>(), 0xffff_ffff);
        const_assert_eq!(mask_u32::<0, 16>(), 0x0000_ffff);
        const_assert_eq!(mask_u32::<16, 32>(), 0xffff_0000);
        const_assert_eq!(mask_u32::<4, 28>(), 0x0fff_fff0);
        const_assert_eq!(mask_u32::<8, 24>(), 0x00ff_ff00);
        const_assert_eq!(mask_u32::<12, 20>(), 0x000f_f000);
        const_assert_eq!(
            mask_u32::<14, 18>(),
        ...
        // u16
        const_assert_eq!(mask_u16::<0, 16>(), 0xffff);
        const_assert_eq!(mask_u16::<0, 8>(), 0x00ff);
        const_assert_eq!(mask_u16::<8, 16>(), 0xff00);
        const_assert_eq!(mask_u16::<4, 12>(), 0x0ff0);
        const_assert_eq!(mask_u16::<6, 10>(), 0b0000_0011_1100_0000);
        const_assert_eq!(mask_u16::<7, 9>(), 0b0000_0001_1000_0000);
        // u8
        const_assert_eq!(mask_u8::<0, 8>(), 0b1111_1111);
        const_assert_eq!(mask_u8::<0, 4>(), 0b0000_1111);
        const_assert_eq!(mask_u8::<4, 8>(), 0b1111_0000);
        const_assert_eq!(mask_u8::<2, 6>(), 0b0011_1100);
        const_assert_eq!(mask_u8::<3, 5>(), 0b0001_1000);
    }
    #[test]
    fn type_max() {
        // u128
        const_assert_eq!(BitRange::<u128, 0, 1>::MAX, 1);
        const_assert_eq!(BitRange::<u128, 1, 3>::MAX, 3);
        const_assert_eq!(BitRange::<u128, 3, 6>::MAX, 7);
        const_assert_eq!(BitRange::<u128, 6, 10>::MAX, 15);
        const_assert_eq!(BitRange::<u128, 10, 15>::MAX, 31);
        const_assert_eq!(BitRange::<u128, 15, 21>::MAX, 63);
        const_assert_eq!(BitRange::<u128, 21, 28>::MAX, 127);
        const_assert_eq!(BitRange::<u128, 28, 36>::MAX, 255);
        const_assert_eq!(BitRange::<u128, 36, 45>::MAX, 511);
        const_assert_eq!(BitRange::<u128, 45, 55>::MAX, 1023);
        const_assert_eq!(BitRange::<u128, 55, 66>::MAX, 2047);
        const_assert_eq!(BitRange::<u128, 66, 78>::MAX, 4095);
        const_assert_eq!(BitRange::<u128, 78, 91>::MAX, 8191);
        const_assert_eq!(BitRange::<u128, 91, 105>::MAX, 16383);
        const_assert_eq!(BitRange::<u128, 105, 120>::MAX, 32767);

        // u64
        const_assert_eq!(BitRange::<u64, 0, 1>::MAX, 1);
        const_assert_eq!(BitRange::<u64, 1, 3>::MAX, 3);
        const_assert_eq!(BitRange::<u64, 3, 6>::MAX, 7);
        const_assert_eq!(BitRange::<u64, 6, 10>::MAX, 15);
        const_assert_eq!(BitRange::<u64, 10, 15>::MAX, 31);
        const_assert_eq!(BitRange::<u64, 15, 21>::MAX, 63);
        const_assert_eq!(BitRange::<u64, 21, 28>::MAX, 127);
        const_assert_eq!(BitRange::<u64, 28, 36>::MAX, 255);
        const_assert_eq!(BitRange::<u64, 36, 45>::MAX, 511);
        const_assert_eq!(BitRange::<u64, 45, 55>::MAX, 1023);

        // u32
        const_assert_eq!(BitRange::<u32, 0, 1>::MAX, 1);
        const_assert_eq!(BitRange::<u32, 1, 3>::MAX, 3);
        const_assert_eq!(BitRange::<u32, 3, 6>::MAX, 7);
        const_assert_eq!(BitRange::<u32, 6, 10>::MAX, 15);
        const_assert_eq!(BitRange::<u32, 10, 15>::MAX, 31);
        const_assert_eq!(BitRange::<u32, 15, 21>::MAX, 63);
        const_assert_eq!(BitRange::<u32, 21, 28>::MAX, 127);

        // u16
        const_assert_eq!(BitRange::<u16, 0, 1>::MAX, 1);
        const_assert_eq!(BitRange::<u16, 1, 3>::MAX, 3);
        const_assert_eq!(BitRange::<u16, 3, 6>::MAX, 7);
        const_assert_eq!(BitRange::<u16, 6, 10>::MAX, 15);
        const_assert_eq!(BitRange::<u16, 10, 15>::MAX, 31);

        const_assert_eq!(BitRange::<u16, 0, 16>::MAX, 65535);
        const_assert_eq!(BitRange::<u16, 0, 8>::MAX, 255);
        const_assert_eq!(BitRange::<u16, 8, 16>::MAX, 255);
        const_assert_eq!(BitRange::<u16, 4, 12>::MAX, 255);
        const_assert_eq!(BitRange::<u16, 6, 10>::MAX, 15);
        const_assert_eq!(BitRange::<u16, 7, 9>::MAX, 3);

        // u8
        const_assert_eq!(BitRange::<u8, 0, 1>::MAX, 1);
        const_assert_eq!(BitRange::<u8, 1, 3>::MAX, 3);
        const_assert_eq!(BitRange::<u8, 3, 6>::MAX, 7);

        const_assert_eq!(BitRange::<u8, 0, 8>::MAX, 255);
        const_assert_eq!(BitRange::<u8, 0, 4>::MAX, 15);
        const_assert_eq!(BitRange::<u8, 4, 8>::MAX, 15);
        const_assert_eq!(BitRange::<u8, 2, 6>::MAX, 15);
        const_assert_eq!(BitRange::<u8, 3, 5>::MAX, 3);
    }
    #[test]
    fn value_max() {
        // These top level checks exists to communicate to code coverage that this function is covered
        assert_eq!(BitFieldu128::new(0).one.get_max(), 1);
        assert_eq!(BitFieldu64::new(0).one.get_max(), 1);
        assert_eq!(BitFieldu32::new(0).one.get_max(), 1);
        assert_eq!(BitFieldu16::new(0).one.get_max(), 1);
        assert_eq!(BitFieldu8::new(0).one.get_max(), 1);

        const _FIELD_U128: BitFieldu128 = BitFieldu128::new(0);
        const _FIELD_U64: BitFieldu64 = BitFieldu64::new(0);
        const _FIELD_U32: BitFieldu32 = BitFieldu32::new(0);
        const _FIELD_U16: BitFieldu16 = BitFieldu16::new(0);
        const _FIELD_U8: BitFieldu8 = BitFieldu8::new(0);

        // u128
        const_assert_eq!(_FIELD_U128.one.get_max(), 1);
        const_assert_eq!(_FIELD_U128.two.get_max(), 3);
        const_assert_eq!(_FIELD_U128.three.get_max(), 7);
        const_assert_eq!(_FIELD_U128.four.get_max(), 15);
        const_assert_eq!(_FIELD_U128.five.get_max(), 31);
        const_assert_eq!(_FIELD_U128.six.get_max(), 63);
        const_assert_eq!(_FIELD_U128.seven.get_max(), 127);
        const_assert_eq!(_FIELD_U128.eight.get_max(), 255);
        const_assert_eq!(_FIELD_U128.nine.get_max(), 511);
        const_assert_eq!(_FIELD_U128.ten.get_max(), 1023);
        const_assert_eq!(_FIELD_U128.eleven.get_max(), 2047);
        const_assert_eq!(_FIELD_U128.twelve.get_max(), 4095);
        const_assert_eq!(_FIELD_U128.thirteen.get_max(), 8191);
        const_assert_eq!(_FIELD_U128.fourteen.get_max(), 16383);
        const_assert_eq!(_FIELD_U128.fifteen.get_max(), 32767);
        // u64
        const_assert_eq!(_FIELD_U64.one.get_max(), 1);
        const_assert_eq!(_FIELD_U64.two.get_max(), 3);
        const_assert_eq!(_FIELD_U64.three.get_max(), 7);
        const_assert_eq!(_FIELD_U64.four.get_max(), 15);
        const_assert_eq!(_FIELD_U64.five.get_max(), 31);
        const_assert_eq!(_FIELD_U64.six.get_max(), 63);
        const_assert_eq!(_FIELD_U64.seven.get_max(), 127);
        const_assert_eq!(_FIELD_U64.eight.get_max(), 255);
        const_assert_eq!(_FIELD_U64.nine.get_max(), 511);
        const_assert_eq!(_FIELD_U64.ten.get_max(), 1023);
        // u32
        const_assert_eq!(_FIELD_U32.one.get_max(), 1);
        const_assert_eq!(_FIELD_U32.two.get_max(), 3);
        const_assert_eq!(_FIELD_U32.three.get_max(), 7);
        const_assert_eq!(_FIELD_U32.four.get_max(), 15);
        const_assert_eq!(_FIELD_U32.five.get_max(), 31);
        const_assert_eq!(_FIELD_U32.six.get_max(), 63);
        const_assert_eq!(_FIELD_U32.seven.get_max(), 127);
        // u16
        const_assert_eq!(_FIELD_U16.one.get_max(), 1);
        const_assert_eq!(_FIELD_U16.two.get_max(), 3);
        const_assert_eq!(_FIELD_U16.three.get_max(), 7);
        const_assert_eq!(_FIELD_U16.four.get_max(), 15);
        const_assert_eq!(_FIELD_U16.five.get_max(), 31);
        // u8
        const_assert_eq!(_FIELD_U8.one.get_max(), 1);
        const_assert_eq!(_FIELD_U8.two.get_max(), 3);
        const_assert_eq!(_FIELD_U8.three.get_max(), 7);
    }
    ...
    fn checked_add_assign() {
        ...
        // We check all values are as expected at tend (we do this to ensure no opreation overflow)
        assert_eq!(bitfield.RANGE1, 1);
        assert_eq!(bitfield.SSE, true);
        assert_eq!(bitfield.SSE1, true);
        assert_eq!(bitfield.RANGE2, 3);

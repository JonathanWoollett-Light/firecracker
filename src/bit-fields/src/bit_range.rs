...
macro_rules! bit_mut_range {
    ($x:ty, $y:ty, $mask:ident, $max:ident) => {
        ...
        impl<const START: u8, const END: u8> BitRangeMut<'_, $y, START, END> {
            // SAFETY: A zero value results in an error at compile time.
            pub const MASK: $y = unsafe { <$y>::new_unchecked($mask::<START, END>()) };
            // SAFETY: A zero value results in an error at compile time.
            /// The maximum value this range can store
            pub const MAX: $y = unsafe { <$y>::new_unchecked($max::<START, END>()) };
            ...
            #[doc = concat!("
                ...
                When `x` is greater than the maximum storable value in `self`.
                ```
                use bit_fields::{BitRangeMut, NonZeroCheckedAssignError};
                use std::num::", stringify!($y) ,";
                let mut x = ", stringify!($y), "::new(18).unwrap();

                let mut nibble = BitRangeMut::<_,0,4>(&mut x);
                assert_eq!(nibble.write(0),Err(NonZeroCheckedAssignError::Zero));
                assert_eq!(nibble.write(16),Err(NonZeroCheckedAssignError::Overflow));
                assert_eq!(nibble.write(15),Ok(()));
            ...
            ")]
            #[inline]
            pub fn write(&mut self, x: $x) -> Result<(), $crate::NonZeroCheckedAssignError> {
                self.checked_assign(x)
            }
            ...
            pub fn checked_add_assign(
                ...
                if x <= Self::MAX.get() {
                    let cur = self.read();
                    if x <= Self::MAX.get() - cur {
                        debug_assert!(cur + x <= Self::MAX.get());
                        // SAFETY: `x <= Self::MAX.get() - cur` implies
                        // `cur + x <= Self::MAX`.
                        unsafe {
                            self.unchecked_assign(cur + x);
                        }
            ...
            #[doc = concat!("
                ...
                1. When `x` is greater than the maximum value storable in the bit range.
                2. When subtracting `x` from the value of the bit range would underflow.
                3. When subtracting `x` from the value of the bit range would zero it.

                ```
                use bit_fields::{BitRangeMut, NonZeroCheckedSubAssignError};
                use std::num::", stringify!($y) ,";
                let mut x = ", stringify!($y), "::new(18).unwrap();

                let mut nibble = BitRangeMut::<_,0,4>(&mut x);
                assert_eq!(nibble,2);
                assert_eq!(nibble.checked_sub_assign(16), Err(NonZeroCheckedSubAssignError::OutOfRange));
                assert_eq!(nibble.checked_sub_assign(3), Err(NonZeroCheckedSubAssignError::Underflow));
                assert_eq!(nibble.checked_sub_assign(1), Ok(()));
                assert_eq!(nibble,1);
                assert_eq!(x.get(),17);
                ...
                assert_eq!(nibble,1);
                assert_eq!(nibble.checked_sub_assign(16), Err(NonZeroCheckedSubAssignError::OutOfRange));
                assert_eq!(nibble.checked_sub_assign(2), Err(NonZeroCheckedSubAssignError::Underflow));
                assert_eq!(nibble.checked_sub_assign(1), Err(NonZeroCheckedSubAssignError::Zero));
                assert_eq!(nibble,1);
            ...
            pub fn checked_sub_assign(
                &mut self,
                x: $x,
            ) -> Result<(), $crate::NonZeroCheckedSubAssignError> {
                if x <= Self::MAX.get() {
                    let cur = self.read();
                    if x < cur {
                        debug_assert!(cur - x > 0);
                        // SAFETY: `x < cur` implies `cur - x > 0`.
                        unsafe {
                            self.unchecked_assign(cur - x);
                        }
                        ...
                        Err($crate::NonZeroCheckedSubAssignError::Zero)
                    }
                    else {
                        debug_assert!(x > cur);
                        Err($crate::NonZeroCheckedSubAssignError::Underflow)
                    }
                } else {
                    Err($crate::NonZeroCheckedSubAssignError::OutOfRange)
                }
            }
            ...
            #[doc = concat!("
                ...
                let mut nibble = BitRangeMut::<_,0,4>(&mut x);
                assert_eq!(nibble.checked_assign(0),Err(NonZeroCheckedAssignError::Zero));
                assert_eq!(nibble.checked_assign(16),Err(NonZeroCheckedAssignError::Overflow));
                assert_eq!(nibble.checked_assign(15),Ok(()));
                assert_eq!(x.get(),31);
            ...
            #[allow(clippy::integer_arithmetic, clippy::arithmetic_side_effects)]
            #[inline]
            pub fn checked_assign(&mut self, x: $x) -> Result<(), $crate::NonZeroCheckedAssignError> {
                if x == 0 {
                    Err($crate::NonZeroCheckedAssignError::Zero)
                }
                else if x > Self::MAX.get() {
                    Err($crate::NonZeroCheckedAssignError::Overflow)
                }
                else {
                    debug_assert!(x <= Self::MAX.get());
                    // SAFETY: Safe due to checking `x <= Self::MAX`.
                    unsafe {
                        self.unchecked_assign(x);
                    }
            ...
            pub unsafe fn unchecked_assign(&mut self, x: $x) {
                ...
                let shift = x << START;
                *self.0 = <$y>::new_unchecked(shift | (self.0.get() & !Self::MASK.get()));
            }
            #[doc = concat!("
                Convenience alias for [`Self::MAX`].
...
macro_rules! bit_range_from {
    ($x:ty, $y:ty, $bit_range: ident) => {
        ...
        // `START < 8 * size_of::<$x>()` is always true so the right shift will not panic.
        #[allow(clippy::integer_arithmetic, clippy::arithmetic_side_effects)]
        impl<const START: u8, const END: u8> From<&$bit_range<'_, $y, START, END>> for $x {
            #[inline]
            fn from(this: &$bit_range<'_, $y, START, END>) -> Self {
                let a = $bit_range::<'_, $y, START, END>::MASK.get() & this.0.get();
                a >> START
            }
        }
    };
...
macro_rules! bit_range_eq {
    ($x:ty, $y:ty, $bit_range: ident) => {
        ...
        impl<const START: u8, const END: u8> PartialEq<$y> for $bit_range<'_, $x, START, END> {
            #[inline]
        ...
        impl<const START: u8, const END: u8> PartialEq<$x> for $bit_range<'_, $y, START, END> {
            #[inline]
            fn eq(&self, other: &$x) -> bool {
                self.read() == *other
            }

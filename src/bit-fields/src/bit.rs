...
macro_rules! bit_display {
    ($x:ty, $y:ty) => {
        ...
        impl<const P: u8> std::fmt::Display for Bit<'_, $y, P> {
            #[doc = concat!("
                ```
                use bit_fields::Bit;
                let x = std::num::", stringify!($y), "::new(5).unwrap();
                assert_eq!(Bit::<_,0>(&x).to_string(),true.to_string());
                assert_eq!(Bit::<_,1>(&x).to_string(),false.to_string());
                assert_eq!(Bit::<_,2>(&x).to_string(),true.to_string());
                ```
...
bit_display!(u128, NonZeroU128);
bit_display!(u64, NonZeroU64);
bit_display!(u32, NonZeroU32);
...
macro_rules! bit_mut_display {
    ($x:ty, $y:ty) => {
        ...
        impl<const P: u8> std::fmt::Display for BitMut<'_, $y, P> {
            #[doc = concat!("
                ```
                use bit_fields::BitMut;
                let mut x = std::num::", stringify!($y), "::new(5).unwrap();
                assert_eq!(BitMut::<_,0>(&mut x).to_string(),true.to_string());
                assert_eq!(BitMut::<_,1>(&mut x).to_string(),false.to_string());
                assert_eq!(BitMut::<_,2>(&mut x).to_string(),true.to_string());
                ```
...
bit_mut_display!(u128, NonZeroU128);
bit_mut_display!(u64, NonZeroU64);
bit_mut_display!(u32, NonZeroU32);
...
macro_rules! bit {
    ($x:ty,$y:ty) => {
        ...
        impl<const P: u8> Bit<'_, $y, P> {
            // SAFETY: Value known to be non-zero at compile time.
            pub const MASK: $y = unsafe { <$y>::new_unchecked(1 << P) };
    ...
    ($x:ty,$y:ty) => {
        ...
        impl<const P: u8> BitMut<'_, $y, P> {
            ...
            #[doc = concat!("
                ...
                let mut x = ", stringify!($y), "::new(5).unwrap();
                assert_eq!(BitMut::<_,0>(&mut x).write(false),Some(()));
                assert_eq!(x,", stringify!($y), "::new(4).unwrap());
                assert_eq!(BitMut::<_,1>(&mut x).write(true),Some(()));
                assert_eq!(x,", stringify!($y), "::new(6).unwrap());
                assert_eq!(BitMut::<_,2>(&mut x).write(false),Some(()));
                assert_eq!(x,", stringify!($y), "::new(2).unwrap());
                ```
            ")]
            #[inline]
            pub fn write(&mut self, x: bool) -> Option<()> {
                self.set(x)
            }
            ...
            #[doc = concat!("
                ...
                let mut x = ", stringify!($y), "::new(5).unwrap();
                BitMut::<_,0>(&mut x).on();
                assert_eq!(x,", stringify!($y), "::new(5).unwrap());
                BitMut::<_,1>(&mut x).on();
            ...
            #[doc = concat!("
                ...
                use std::num::", stringify!($y), ";
                let mut x = ", stringify!($y), "::new(5).unwrap();
                assert_eq!(BitMut::<_,0>(&mut x).off(),Some(()));
                assert_eq!(x,", stringify!($y), "::new(4).unwrap());
                assert_eq!(BitMut::<_,1>(&mut x).off(),Some(()));
                assert_eq!(x,", stringify!($y), "::new(4).unwrap());
                assert_eq!(BitMut::<_,2>(&mut x).off(),None);
                assert_eq!(x,", stringify!($y), "::new(4).unwrap());
                ```
            ")]
            #[allow(clippy::integer_arithmetic, clippy::arithmetic)]
            ...
            #[doc = concat!("
                ...
                use std::num::", stringify!($y), ";
                let mut x = ", stringify!($y), "::new(5).unwrap();
                assert_eq!(BitMut::<_,0>(&mut x).flip(),Some(()));
                assert_eq!(x,", stringify!($y), "::new(4).unwrap());
                assert_eq!(BitMut::<_,1>(&mut x).flip(),Some(()));
                assert_eq!(x,", stringify!($y), "::new(6).unwrap());
                assert_eq!(BitMut::<_,2>(&mut x).flip(),Some(()));
                assert_eq!(x,", stringify!($y), "::new(2).unwrap());
                ```
            ")]
            #[allow(clippy::integer_arithmetic, clippy::arithmetic)]
            ...
            ")]
            #[inline]
            pub fn set(&mut self, x: bool) -> Option<()> {
                if x {
                    self.on();
                    Some(())
                } else {
...
bit_mut!(u128, NonZeroU128);
bit_mut!(u64, NonZeroU64);
bit_mut!(u32, NonZeroU32);
...
macro_rules! bit_from {
    ($x:ty,$y:ty) => {
        ...
        impl<const P: u8> From<&Bit<'_, $y, P>> for bool {
            ...
            ")]
            #[inline]
            fn from(this: &Bit<'_, $y, P>) -> Self {
                ((*this.0).get() & Bit::<$x, P>::MASK) != 0
            }
        }
    };
}
bit_from!(u128, NonZeroU128);
bit_from!(u64, NonZeroU64);
bit_from!(u32, NonZeroU32);
...
macro_rules! bit_mut_from {
    ($x:ty,$y:ty) => {
        ...
        impl<const P: u8> From<&BitMut<'_, $y, P>> for bool {
            ...
            ")]
            #[inline]
            fn from(this: &BitMut<'_, $y, P>) -> Self {
                (this.0.get() & BitMut::<$x, P>::MASK) != 0
            }
        }
    };
}

bit_mut_from!(u128, NonZeroU128);
bit_mut_from!(u64, NonZeroU64);
bit_mut_from!(u32, NonZeroU32);
...
macro_rules! bit_eq {
    ($x:ty, $y:ty) => {
        ...
        impl<const P: u8> PartialEq for Bit<'_, $y, P> {
            #[doc = concat!("
                ```
                use bit_fields::Bit;
                use std::num::", stringify!($y), ";
                let x = ", stringify!($y), "::new(5).unwrap();
                let y = ", stringify!($y), "::new(5).unwrap();
                let a = Bit::<_,0>(&x);
                let b = Bit::<_,0>(&y);
        ...
        impl<const P: u8> PartialEq<bool> for Bit<'_, $y, P> {
            #[doc = concat!("
                ```
                use bit_fields::Bit;
                let x = std::num::", stringify!($y), "::new(5).unwrap();
        ...
        impl<const P: u8> Eq for Bit<'_, $y, P> {}
    };
}

bit_eq!(u128, NonZeroU128);
bit_eq!(u64, NonZeroU64);
bit_eq!(u32, NonZeroU32);
...
macro_rules! bit_mut_eq {
    ($x:ty, $y:ty) => {
        ...
        impl<const P: u8> PartialEq for BitMut<'_, $y, P> {
            #[doc = concat!("
                ```
                use bit_fields::BitMut;
                use std::num::", stringify!($y), ";
                let mut x = ", stringify!($y), "::new(5).unwrap();
                let mut y = ", stringify!($y), "::new(5).unwrap();
                let a = BitMut::<_,0>(&mut x);
                let b = BitMut::<_,0>(&mut y);
        ...
        impl<const P: u8> PartialEq<bool> for BitMut<'_, $y, P> {
            #[doc = concat!("
                ```
                use bit_fields::BitMut;
                let mut x = std::num::", stringify!($y), "::new(5).unwrap();
        ...
        impl<const P: u8> Eq for BitMut<'_, $y, P> {}
    };
}

bit_mut_eq!(u128, NonZeroU128);
bit_mut_eq!(u64, NonZeroU64);
bit_mut_eq!(u32, NonZeroU32);
bit_mut_eq!(u16, NonZeroU16);
bit_mut_eq!(u8, NonZeroU8);

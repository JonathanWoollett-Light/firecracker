...
use logger::{error, info, LoggerConfig, ProcessTimeReporter, StoreMetric, LOGGER, METRICS};
...
enum MainError {
    ...
    SetLogger(logger::LoggerInitError),
    ...
    InvalidLogLevel(logger::LevelFilterFromStrError),
    /// Could not initialize logger: {0}
    LoggerInitialization(logger::LoggerUpdateError),
...
fn main_exec() -> Result<(), MainError> {
    ...
    logger::INSTANCE_ID.set(String::from(id)).unwrap();

    // Apply the logger configuration.
    let log_path = arguments.single_value("log-path").map(PathBuf::from);
    let level = arguments
        .single_value("level")
        .map(|s| logger::LevelFilter::from_str(s))
        .transpose()

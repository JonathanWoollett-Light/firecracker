...
pub use cpuid_ffi::*;
pub use intel::{IntelCpuid, UnknownLeaf};
...
impl Cpuid {
    ...
    /// Returns the CPUID manufacturers ID. E.g. `GenuineIntel` or `AuthenticAMD`.
    #[must_use]
    pub fn manufacturer_id(&self) -> Result<FixedString<12>, crate::intel::MissingLeaf0> {
        match self {
            Self::Intel(intel) => intel.manufacturer_id(),
            Self::Amd(amd) => Ok(amd.manufacturer_id()),
        }
    }
...
impl<T: FeatureComparison> FeatureComparison for &T {
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
        (**self).feature_cmp(other)
    }
}
impl<T: FeatureComparison> FeatureComparison for &mut T {
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
        (**self).feature_cmp(other)
    }
}
...
impl TryFrom<Cpuid> for RawCpuid {
    type Error = UnknownLeaf;
    fn try_from(cpuid: Cpuid) -> Result<Self, Self::Error> {
        match cpuid {
            Cpuid::Intel(intel_cpuid) => RawCpuid::try_from(intel_cpuid),
            Cpuid::Amd(amd_cpuid) => Ok(RawCpuid::from(amd_cpuid)),
        }
    }
}
#[cfg(cpuid)]
impl TryFrom<Cpuid> for kvm_bindings::CpuId {
    type Error = UnknownLeaf;
    fn try_from(cpuid: Cpuid) -> Result<Self, Self::Error> {
        let raw_cpuid = RawCpuid::try_from(cpuid)?;
        Ok(Self::from(raw_cpuid))
    }
}
...
mod tests {
    ...
    fn kvm_native_compare() {
        ...
        // Superset
        assert!(native_intel.leaf_1.ecx() & kvm_intel.leaf_1.ecx() == kvm_intel.leaf_1.ecx());
        assert!(native_intel.leaf_1.edx() & kvm_intel.leaf_1.edx() == kvm_intel.leaf_1.edx());
        ...
        assert_eq!(
            kvm_intel.leaf_4.0[n],
            intel::Leaf4Subleaf::from(
                intel::Leaf4Eax::from(0),
                intel::Leaf4Ebx::from(0),
                intel::Leaf4Ecx::from(0),
                intel::Leaf4Edx::from(0),
            )
        );

        // Check for terminating leaf 5 with all zero registers
        assert_eq!(kvm_intel.leaf_5, None);
        assert!(matches!(
            native_intel.leaf_6.feature_cmp(&kvm_intel.leaf_6),
            Some(FeatureRelation::Superset | FeatureRelation::Equal)
        ));

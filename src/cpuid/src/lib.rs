...
pub enum KvmGetSupportedCpuidError {
    ...
    CpuidFromRaw(#[from] CpuidFromRawCpuidError),
}
...
impl Cpuid {
    ...
    #[cfg(cpuid)]
    pub unsafe fn new() -> Result<Cpuid, UnsupportedManufacturer> {
        ...
        // convert.
        let cmp_str: &str = std::str::from_utf8_unchecked(&manufacturer_str.0);

        match cmp_str {
            "GenuineIntel" => Ok(Cpuid::Intel(IntelCpuid::new())),
            "AuthenticAMD" => Ok(Cpuid::Amd(AmdCpuid::new())),
            _ => Err(UnsupportedManufacturer(manufacturer_str)),
        }
    }
...
/// Error type for [`<Cpuid as TryFrom<RawCpuid>>::from()`].
#[derive(Debug, thiserror::Error)]
pub enum CpuidFromRawCpuidError {
    /// Unknown CPUID manufacturer id.
    #[error("Unknown CPUID manufacturer id: {0}")]
    UnknownManufacturer(UnsupportedManufacturer),
    /// Missing required CPUID leaf.
    #[error("Missing required CPUID leaf: {0}")]
    MissingLeaf(#[from] intel::MissingLeaf),
}

impl TryFrom<RawCpuid> for Cpuid {
    type Error = CpuidFromRawCpuidError;
    fn try_from(raw_cpuid: RawCpuid) -> Result<Self, Self::Error> {
        ...
        match manufacturer_str {
            "GenuineIntel" => Ok(Cpuid::Intel(IntelCpuid::try_from(raw_cpuid)?)),
            "AuthenticAMD" => Ok(Cpuid::Amd(AmdCpuid::from(raw_cpuid))),
            _ => Err(CpuidFromRawCpuidError::UnknownManufacturer(
                UnsupportedManufacturer(FixedString(arr)),
            )),
        }
    }
}

...
pub enum Leaf0NotSupported {
    /// Maximum input value.
    #[error("Maximum input value.")]
    MaximumInputValue,
    /// Manufacturer ID.
    #[error("Manufacturer ID.")]
    ManufacturerId,
}
...
impl Supports for Leaf7<'_> {
    ...
    fn supports(&self, other: &Self) -> Result<(), Self::Error> {
        match (self.0, other.0) {
            (None, None) => (),
            (Some(_), None) | (None, Some(_)) => return Err(Leaf7NotSupported::MissingSubleaf0),
            (Some(a), Some(b)) => a.supports(b).map_err(Leaf7NotSupported::Subleaf0)?,
        }
        match (self.1, other.1) {
            (None, None) => (),
            (Some(_), None) | (None, Some(_)) => return Err(Leaf7NotSupported::MissingSubleaf1),
...
impl Supports for Leaf7Subleaf0 {
    ...
    fn supports(&self, other: &Self) -> Result<(), Self::Error> {
        ...
        if matches!(self.ebx.cmp_flags(&other.ebx), Some(Ordering::Less) | None) {
            return Err(Leaf7Subleaf0NotSupported::Ebx);
        }
        if matches!(self.ecx.cmp_flags(&other.ecx), Some(Ordering::Less) | None) {
            return Err(Leaf7Subleaf0NotSupported::Ecx);
        }
        if matches!(self.edx.cmp_flags(&other.edx), Some(Ordering::Less) | None) {
            return Err(Leaf7Subleaf0NotSupported::Edx);
        }
...
impl Supports for LeafF<'_> {
    ...
    fn supports(&self, other: &Self) -> Result<(), Self::Error> {
        match (self.0, other.0) {
            (None, None) => (),
            (Some(_), None) | (None, Some(_)) => return Err(LeafFNotSupported::MissingSubleaf0),
            (Some(a), Some(b)) => a.supports(b).map_err(LeafFNotSupported::Subleaf0)?,
        }
        match (self.1, other.1) {
            (None, None) => (),
            (Some(_), None) | (None, Some(_)) => return Err(LeafFNotSupported::MissingSubleaf1),
...
impl Supports for Leaf10<'_> {
    ...
    fn supports(&self, other: &Self) -> Result<(), Self::Error> {
        ...
        match (self.0, other.0) {
            (None, None) => (),
            (Some(_), None) | (None, Some(_)) => return Err(Leaf10NotSupported::MissingSubleaf0),
            (Some(a), Some(b)) => a.supports(b).map_err(Leaf10NotSupported::Subleaf0)?,
        }
        match (self.1, other.1) {
            (None, None) => (),
            (Some(_), None) | (None, Some(_)) => return Err(Leaf10NotSupported::MissingSubleaf1),
            (Some(a), Some(b)) => a.supports(b).map_err(Leaf10NotSupported::Subleaf1)?,
        }
        match (self.3, other.3) {
            (None, None) => (),
            (Some(_), None) | (None, Some(_)) => return Err(Leaf10NotSupported::MissingSubleaf3),
...
impl Supports for Leaf14<'_> {
    ...
    fn supports(&self, other: &Self) -> Result<(), Self::Error> {
        ...
        match (self.0, other.0) {
            (None, None) => (),
            (Some(_), None) | (None, Some(_)) => return Err(Leaf14NotSupported::MissingSubleaf0),
...
impl Supports for Leaf18<'_> {
    ...
    fn supports(&self, other: &Self) -> Result<(), Self::Error> {
        ...
        match (self.0.as_ref(), other.0.as_ref()) {
            (None, None) => (),
            (Some(_), None) | (None, Some(_)) => return Err(Leaf18NotSupported::MissingSubleaf0),

...
use super::*;
use crate::{cascade_cpo, KvmCpuidFlags, RawKvmCpuidEntry};
...
impl From<(u32, u32, u32, u32)> for Leaf2 {
    fn from((eax, ebx, ecx, edx): (u32, u32, u32, u32)) -> Self {
        Self(unsafe {
            Leaf {
                eax: std::mem::transmute(eax),
                ebx: std::mem::transmute(ebx),
                ecx: std::mem::transmute(ecx),
                edx: std::mem::transmute(edx),
            }
        })
    }
}
...
/// Leaf 02H
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize, Inline)]
pub struct Leaf2(pub Leaf<[u8; 4], [u8; 4], [u8; 4], [u8; 4]>);
/// Leaf 03H
pub type Leaf3 = Leaf<Leaf3Eax, Leaf3Ebx, Leaf3Ecx, Leaf3Edx>;
/// Leaf 04H wrapper
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize, Inline)]
pub struct Leaf4(pub Vec<Leaf4Subleaf>);
/// Leaf 04H
...
/// Leaf 0BH wrapper
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize, Inline)]
pub struct LeafB(pub Vec<LeafBSubleaf>);
/// Leaf 0BH
...
/// Leaf 18H
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize, Inline)]
pub struct Leaf18(pub Leaf18Subleaf0, pub Vec<Leaf18SubleafGt0>);
/// Leaf 18H subleaf 0
...
/// Leaf 1FH wrapper
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize, Inline)]
pub struct Leaf1F(pub Vec<Leaf1FSubleaf>);
/// Leaf 1FH
...
impl Leaf4 {
    ...
    #[must_use]
    pub fn equal(&self, other: &Self) -> bool {
        self.0.len() == other.0.len() && self.0.iter().zip(other.0.iter()).all(|(a, b)| a.equal(b))
    }
}
impl Leaf4Subleaf {
    /// Compares `self` to `other` ignoring undefined bits.
...
impl Leaf10 {
    ...
    pub fn equal(&self, other: &Self) -> bool {
        self.0.equal(&other.0)
            ...
            && match (self.2.as_ref(), other.2.as_ref()) {
                (Some(_), None) | (None, Some(_)) => false,
                (None, None) => true,
                (Some(a), Some(b)) => a.equal(b),
            }
...
impl Leaf18 {
    ...
    pub fn equal(&self, other: &Self) -> bool {
        self.0.equal(&other.0)
            ...
            && self.1.iter().zip(other.1.iter()).all(|(a, b)| a.equal(b))
    }
}
impl Leaf18Subleaf0 {
    /// Compares `self` to `other` ignoring undefined bits.
...
impl FeatureComparison for Leaf1 {
    ...
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
        ...
        let clflush_cmp = self
            .ebx
            .clflush()
            .partial_cmp(&other.ebx.clflush())
            .map(FeatureRelation::from);

        let max_addressable_logical_processor_ids_cmp = self
            .ebx
        ...
        let ecx_cmp = self_ecx_masked
            .cmp_flags(&other_ecx_masked)
            .map(FeatureRelation::from);
        ...
        let edx_cmp = self_edx_masked
            .cmp_flags(&other_edx_masked)
            .map(FeatureRelation::from);

        cascade_cpo!(
            clflush_cmp,
            max_addressable_logical_processor_ids_cmp,
            ecx_cmp,
            edx_cmp
        )
    }
...
impl FeatureComparison for Leaf5 {
    ...
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
        ...
        let smallest_monitor_line_size_cmp = self
            .eax
        ...
        let largest_monitor_line_size_cmp = self
            .ebx
            .largest_monitor_line_size()
            .partial_cmp(&other.ebx.largest_monitor_line_size())
            .map(FeatureRelation::from);

        let ecx_cmp = self.ecx.cmp_flags(&other.ecx).map(FeatureRelation::from);

        cascade_cpo!(
            smallest_monitor_line_size_cmp,
            largest_monitor_line_size_cmp,
            ecx_cmp
        )
    }
}
...
macro_rules! raw_from_leaf {
    ($leaf: ty, $function: literal, $index: literal, $flags: expr) => {
        impl From<$leaf> for RawKvmCpuidEntry {
...
raw_from_leaf!(Leaf0, 0x0, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf1, 0x1, 0, KvmCpuidFlags::from(0));
impl From<Leaf2> for RawKvmCpuidEntry {
    fn from(leaf: Leaf2) -> Self {
        RawKvmCpuidEntry {
            function: 0x2,
            index: 0,
            flags: KvmCpuidFlags::from(0),
            eax: unsafe { std::mem::transmute(leaf.0.eax) },
            ebx: unsafe { std::mem::transmute(leaf.0.ebx) },
            ecx: unsafe { std::mem::transmute(leaf.0.ecx) },
            edx: unsafe { std::mem::transmute(leaf.0.edx) },
            padding: Padding::default(),
        }
    }
}
raw_from_leaf!(Leaf3, 0x3, 0, KvmCpuidFlags::from(0));
impl From<Leaf4> for Vec<RawKvmCpuidEntry> {
    fn from(leaf: Leaf4) -> Self {
        leaf.0
            .into_iter()
            .enumerate()
            .map(|(index, subleaf)| RawKvmCpuidEntry {
                function: 0x4,
                // For this to panic CPUID would need to have more than 2^32 subleaves here.
                // We can reasonably expect this will never happen.
                index: u32::try_from(index).unwrap(),
                flags: KvmCpuidFlags::SignificantIndex,
                eax: subleaf.eax.into(),
                ebx: subleaf.ebx.into(),
                ecx: subleaf.ecx.into(),
                edx: subleaf.edx.into(),
                padding: Padding::default(),
            })
            .collect()
    }
}
raw_from_leaf!(Leaf5, 0x5, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf6, 0x6, 0, KvmCpuidFlags::from(0));
impl From<Leaf7> for Vec<RawKvmCpuidEntry> {
    fn from(leaf: Leaf7) -> Self {
        let mut vec = vec![RawKvmCpuidEntry {
            function: 0x7,
            index: 0,
            flags: KvmCpuidFlags::SignificantIndex,
            eax: leaf.0.eax.into(),
        ...
        if let Some(subleaf_1) = leaf.1 {
            vec.push(RawKvmCpuidEntry {
                function: 0x7,
                index: 1,
                flags: KvmCpuidFlags::SignificantIndex,
                eax: subleaf_1.eax.into(),
                ebx: subleaf_1.ebx.into(),
                ecx: subleaf_1.ecx.into(),
                edx: subleaf_1.edx.into(),
                padding: Padding::default(),
            });
        }
        vec
...
raw_from_leaf!(Leaf9, 0x9, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(LeafA, 0xA, 0, KvmCpuidFlags::from(0));
impl From<LeafB> for Vec<RawKvmCpuidEntry> {
    fn from(leaf: LeafB) -> Self {
        leaf.0
            .into_iter()
            .enumerate()
            .map(|(index, subleaf)| RawKvmCpuidEntry {
...
impl From<LeafF> for Vec<RawKvmCpuidEntry> {
    fn from(leaf: LeafF) -> Self {
        let mut vec = vec![RawKvmCpuidEntry {
            function: 0xF,
            index: 0,
            flags: KvmCpuidFlags::SignificantIndex,
...
impl From<Leaf10> for Vec<RawKvmCpuidEntry> {
    fn from(leaf: Leaf10) -> Self {
        let mut vec = vec![RawKvmCpuidEntry {
            function: 0x10,
            index: 0,
            flags: KvmCpuidFlags::SignificantIndex,
        ...
        if let Some(subleaf_2) = leaf.2 {
            vec.push(RawKvmCpuidEntry {
                function: 0xF,
                index: 2,
                flags: KvmCpuidFlags::SignificantIndex,
                eax: subleaf_2.eax.into(),
                ebx: subleaf_2.ebx.into(),
                ecx: subleaf_2.ecx.into(),
                edx: subleaf_2.edx.into(),
                padding: Padding::default(),
            });
        }
        if let Some(subleaf_3) = leaf.3 {
            vec.push(RawKvmCpuidEntry {
                function: 0xF,
                index: 3,
                flags: KvmCpuidFlags::SignificantIndex,
                eax: subleaf_3.eax.into(),
                ebx: subleaf_3.ebx.into(),
                ecx: subleaf_3.ecx.into(),
                edx: subleaf_3.edx.into(),
                padding: Padding::default(),
            });
        }
        vec
...
impl From<Leaf12> for Vec<RawKvmCpuidEntry> {
    fn from(leaf: Leaf12) -> Self {
        let mut vec = vec![
            RawKvmCpuidEntry {
                function: 0x12,
                // For this to panic CPUID would need to have more than 2^32 subleaves here.
        ...
        let mut temp = leaf
            ...
            .map(|(index, subleaf)| RawKvmCpuidEntry {
                ...
                // For this to panic CPUID would need to have more than 2^32 subleaves here.
                // We can reasonably expect this will never happen.
                index: u32::try_from(index).unwrap() + 2,
                flags: KvmCpuidFlags::SignificantIndex,
                eax: subleaf.eax.into(),
                ebx: subleaf.ebx.into(),
                ecx: subleaf.ecx.into(),
...
impl From<Leaf14> for Vec<RawKvmCpuidEntry> {
    fn from(leaf: Leaf14) -> Self {
        let mut vec = vec![RawKvmCpuidEntry {
            function: 0x14,
            index: 0,
            flags: KvmCpuidFlags::SignificantIndex,
...
raw_from_leaf!(Leaf15, 0x15, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf16, 0x16, 0, KvmCpuidFlags::from(0));
impl From<Leaf17> for Vec<RawKvmCpuidEntry> {
    fn from(leaf: Leaf17) -> Self {
        vec![
            RawKvmCpuidEntry {
                function: 0x17,
                index: 0,
...
impl From<Leaf18> for Vec<RawKvmCpuidEntry> {
    fn from(leaf: Leaf18) -> Self {
        let mut vec = vec![RawKvmCpuidEntry {
            function: 0x18,
            index: 0,
            flags: KvmCpuidFlags::SignificantIndex,
        ...
        let mut temp = leaf
            ...
            .map(|(index, subleaf)| RawKvmCpuidEntry {
                ...
                // For this to panic CPUID would need to have more than 2^32 subleaves here.
                // We can reasonably expect this will never happen.
                index: u32::try_from(index).unwrap() + 1,
                flags: KvmCpuidFlags::SignificantIndex,
                eax: subleaf.eax.into(),
                ebx: subleaf.ebx.into(),
                ecx: subleaf.ecx.into(),
...
raw_from_leaf!(Leaf19, 0x19, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf1A, 0x1A, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf1B, 0x1B, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf1C, 0x1C, 0, KvmCpuidFlags::from(0));
impl From<Leaf1F> for Vec<RawKvmCpuidEntry> {
    fn from(leaf: Leaf1F) -> Self {
        leaf.0
            .into_iter()
            .enumerate()
            .map(|(index, subleaf)| RawKvmCpuidEntry {
...
raw_from_leaf!(Leaf20, 0x20, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf80000000, 0x80000000, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf80000001, 0x80000001, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf80000002, 0x80000002, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf80000005, 0x80000005, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf80000006, 0x80000006, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf80000007, 0x80000007, 0, KvmCpuidFlags::from(0));
raw_from_leaf!(Leaf80000008, 0x80000008, 0, KvmCpuidFlags::from(0));

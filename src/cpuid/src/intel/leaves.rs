...
/// The KVM requires a `flags` paramter which indicates if a given CPUID leaf has subleaves. This
/// does not change at runtime so we can save memory by not storing this under every subleaf and
/// instead fetching from a map when converting back to the KVM CPUID structure.
...
// #[cfg(cpuid)]
// impl<A: From<u32> ,B: From<u32> ,C: From<u32>, D: From<u32>> From<CpuidResult> for Leaf<A, B, C, D> {
//     fn from(CpuidResult { eax, ebx, ecx, edx }: CpuidResult) -> Self {
//         Self(Leaf { eax: A::from(eax), ebx: ebx::from(ebx), ecx: ecx::from(ecx), edx: edx::from(edx) })
//     }
// }
...
pub struct Leaf4<'a>(pub &'a [Leaf4Subleaf]);
#[derive(Debug, PartialEq, Eq)]
pub struct Leaf4Mut<'a>(pub &'a mut [Leaf4Subleaf]);
...
/// Leaf 0BH
#[derive(Debug, PartialEq, Eq)]
pub struct LeafB<'a>(pub &'a [LeafBSubleaf]);
#[derive(Debug, PartialEq, Eq)]
pub struct LeafBMut<'a>(pub &'a mut [LeafBSubleaf]);
pub type LeafBSubleaf = Leaf<LeafBEax, LeafBEbx, LeafBEcx, LeafBEdx>;

// /// Leaf 0DH
// pub type LeafD<'a> = (Option<&'a LeafDSubleaf0>, Option<&'a LeafDSubleaf1>, &'a [LeafDSubleafGt1]);
// pub type LeafDMut<'a> = (Option<&'a LeafDSubleaf0>, Option<&'a LeafDSubleaf1>, &'a [LeafDSubleafGt1]);
...
/// Leaf 18H
#[derive(Debug, PartialEq, Eq)]
pub struct Leaf18<'a>(pub Option<&'a Leaf18Subleaf0>, pub &'a [Leaf18SubleafGt0]);
#[derive(Debug, PartialEq, Eq)]
pub struct Leaf18Mut<'a>(
    pub Option<&'a mut Leaf18Subleaf0>,
    pub &'a mut [Leaf18SubleafGt0],
);
...
/// Leaf 1FH
#[derive(Debug, PartialEq, Eq)]
pub struct Leaf1F<'a>(pub &'a [Leaf1FSubleaf]);
#[derive(Debug, PartialEq, Eq)]
pub struct Leaf1FMut<'a>(pub &'a mut [Leaf1FSubleaf]);
...
impl FeatureComparison for Leaf7<'_> {
    #[logfn(Trace)]
    #[logfn_inputs(Trace)]
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
...
impl FeatureComparison for Leaf7Mut<'_> {
    #[logfn(Trace)]
    #[logfn_inputs(Trace)]
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
        cpo(self.0.feature_cmp(&other.0), self.1.feature_cmp(&other.1))
...
impl FeatureComparison for LeafF<'_> {
...
impl FeatureComparison for LeafFMut<'_> {
    /// We check sub-leaves 0 and 1.
    #[logfn(Trace)]
    #[logfn_inputs(Trace)]
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
...
impl FeatureComparison for Leaf10<'_> {
    #[logfn(Trace)]
    #[logfn_inputs(Trace)]
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
        log::warn!(
...
impl FeatureComparison for Leaf10Mut<'_> {
    #[logfn(Trace)]
    #[logfn_inputs(Trace)]
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
        log::warn!(
...
impl FeatureComparison for Leaf14<'_> {
...
impl FeatureComparison for Leaf14Mut<'_> {
    /// Only checks subleaf 1.
    #[logfn(Trace)]
    #[logfn_inputs(Trace)]
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
...
impl FeatureComparison for Leaf18<'_> {
    #[logfn(Trace)]
    #[logfn_inputs(Trace)]
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
        log::warn!(
...
impl FeatureComparison for Leaf18Mut<'_> {
    #[logfn(Trace)]
    #[logfn_inputs(Trace)]
    fn feature_cmp(&self, other: &Self) -> Option<FeatureRelation> {
        log::warn!(

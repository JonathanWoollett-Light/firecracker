...
impl KvmVcpu {
    ...
    pub fn configure(
        ...
        let extra_msrs = cpuid::common::msrs_to_save_by_cpuid(&cpuid)
        ...
        if vcpu_config.cpu_template == CpuFeaturesTemplate::T2S {
            self.msr_list.extend(cpuid::t2s::msr_entries_to_save());
...
mod tests {
    ...
    fn test_configure_vcpu() {
        ...
        match &get_vendor_id_from_host().unwrap() {
            ...
            _ => {
                let err = Err(KvmVcpuConfigureError::KvmUnsupportedCpuid(
                    cpuid::CpuidNotSupported::Incompatible,
                ));
                assert_eq!(t2_res, err);
                assert_eq!(c3_res, err);
                assert_eq!(t2s_res, err);
            }
        }
    }

...
impl From<OpCode> for &'static str {
    #[tracing::instrument(level = "info", skip(opcode))]
...
impl<T: Debug> Operation<T> {
    #[tracing::instrument(level = "info", skip(fd, addr, len, offset, user_data))]
    ...
    #[tracing::instrument(level = "info", skip(fd, user_data))]

...
#[tracing::instrument(level = "info", skip(path, file_engine_type))]
...
#[tracing::instrument(level = "info", skip(blk, idx, q))]
pub fn set_queue(blk: &mut Block, idx: usize, q: Queue) {
    blk.queues[idx] = q;
}

#[tracing::instrument(level = "info", skip(blk, rl))]
pub fn set_rate_limiter(blk: &mut Block, rl: RateLimiter) {
    blk.rate_limiter = rl;
}

#[tracing::instrument(level = "info", skip(blk))]
pub fn rate_limiter(blk: &mut Block) -> &RateLimiter {
    &blk.rate_limiter
}

#[tracing::instrument(level = "info", skip(b, maybe_expected_irq))]
#[cfg(test)]
...
#[tracing::instrument(level = "info", skip(b, expected_irq))]
#[cfg(test)]
...
impl<'a, 'b> RequestDescriptorChain<'a, 'b> {
    #[tracing::instrument(level = "info", skip(vq))]
    ...
    #[tracing::instrument(level = "info", skip(self, header))]

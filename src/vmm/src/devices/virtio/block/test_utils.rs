...
#[tracing::instrument(level = "trace", ret(skip), skip(file_engine_type))]
...
#[tracing::instrument(level = "trace", ret(skip), skip(path,file_engine_type))]
...
#[tracing::instrument(level = "trace", ret(skip), skip(blk,idx,q))]
pub fn set_queue(blk: &mut Block, idx: usize, q: Queue) {
    blk.queues[idx] = q;
}

#[tracing::instrument(level = "trace", ret(skip), skip(blk,rl))]
pub fn set_rate_limiter(blk: &mut Block, rl: RateLimiter) {
    blk.rate_limiter = rl;
}

#[tracing::instrument(level = "trace", skip(blk))]
pub fn rate_limiter(blk: &mut Block) -> &RateLimiter {
    &blk.rate_limiter
}

#[tracing::instrument(level = "trace", ret(skip), skip(b,maybe_expected_irq))]
#[cfg(test)]
...
#[tracing::instrument(level = "trace", ret(skip), skip(b,expected_irq))]
#[cfg(test)]
...
impl<'a, 'b> RequestDescriptorChain<'a, 'b> {
    #[tracing::instrument(level = "trace", ret(skip), skip(vq))]
    ...
    #[tracing::instrument(level = "trace", ret(skip), skip(self,header))]

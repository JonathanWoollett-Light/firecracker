        ...
        if identifier_str_upper_snake != identifier_str {
            ...
            if data_type.is_nonzero() {
                self.flag_constants.extend(quote! {
                    #rustdoc_stream
                    // SAFETY: Always safe.
                    pub const #ident_upper_snake: Self = Self(unsafe { <#data_type>::new_unchecked(1 <<  #index) });
                });
            } else {
                self.flag_constants.extend(quote! {
                    #rustdoc_stream
    ...
    pub fn compose(mut self) -> TokenStream {
        ...
        let set_theory = {
            let (superset, subset, disjoint, intersection, union_fn) = if data_type.is_nonzero() {
                (
                    quote! { (self.0.get() & other.0.get() & Self::BIT_FLAG_MASK) == (other.0.get() & Self::BIT_FLAG_MASK) },
                    quote! { (self.0.get() & other.0.get() & Self::BIT_FLAG_MASK) == (self.0.get() & Self::BIT_FLAG_MASK) },
                    quote! { (self.0.get() & other.0.get() & Self::BIT_FLAG_MASK) == 0 },
                    quote! {
                        ...
                        /// fields which would result in the underlying `NonZero` data type being 0.
                        /// In this case `None` is returned.
                        #[allow(clippy::bad_bit_mask)]
                        pub fn intersection(&self, other: &Self) -> Option<Self> {
                            #data_type::new(self.0.get() & other.0.get() & Self::BIT_FLAG_MASK).map(Self)
                        }
                    },
                    quote! {
                        ...
                        /// # Safety
                        ///
                        /// A union of 2 bit fields with 0 bit flags returns a union with 0 fields,
                        /// this would result in the underlying `NonZero` data type being 0. In this
                ...
                (
                    quote! { (self.0 & other.0 & Self::BIT_FLAG_MASK) == (other.0 & Self::BIT_FLAG_MASK) },
                    quote! { (self.0 & other.0 & Self::BIT_FLAG_MASK) == (self.0 & Self::BIT_FLAG_MASK) },
                    quote! { (self.0 & other.0 & Self::BIT_FLAG_MASK) == 0 },
                    quote! {
                        /// Returns the [`intersection`](https://en.wikipedia.org/wiki/Intersection_(set_theory)) of `self` and `other`.
                        ///
                        /// Returns the intersection of defined bit flags, all other bits will be 0.
            ...
            if data_type.is_nonzero() {
                quote! {
                    impl<T:std::fmt::Display> std::convert::TryFrom<std::collections::HashSet<T>> for #struct_name {
                        type Error = bit_fields::NonZeroTryFromFlagSetError;
                        fn try_from(set: std::collections::HashSet<T>) -> Result<Self,Self::Error> {
                            let value = set.into_iter().try_fold(0,|mut acc,key| match key.to_string().as_str() {
                                #flag_matching_from_hashset
                                _ => Err(bit_fields::NonZeroTryFromFlagSetError::MissingFlag)
                            })?;
                            #data_type::new(value).map(Self).ok_or(bit_fields::NonZeroTryFromFlagSetError::ZeroFlags)
                        }
                    }
                }
            } else {
            ...
            if data_type.is_nonzero() {
                quote! {
                    impl<T:std::fmt::Display, K> std::convert::TryFrom<std::collections::HashMap<T,K>> for #struct_name where #base: From<K> {
                        type Error = bit_fields::NonZeroTryFromFieldMapError;
                        fn try_from(map: std::collections::HashMap<T,K>) -> Result<Self,Self::Error> {
                            let value = map.into_iter().try_fold(0,|mut acc,(k,v)|match k.to_string().as_str() {
                                #field_matching_from_hashmap
                                _ => Err(bit_fields::NonZeroTryFromFieldMapError::UnknownRange)
                            })?;
                            #data_type::new(value).map(Self).ok_or(bit_fields::NonZeroTryFromFieldMapError::ZeroRanges)
                        }
                    }
                }
            } else {
            ...
            if data_type.is_nonzero() {
                quote! {
                    impl<T:std::fmt::Display, K> std::convert::TryFrom<(std::collections::HashSet<T>,std::collections::HashMap<T,K>)> for #struct_name where #base: From<K> {
                        type Error = bit_fields::NonZeroTryFromFlagSetAndFieldMapError;
                        fn try_from((set,map):(std::collections::HashSet<T>,std::collections::HashMap<T,K>)) -> Result<Self,Self::Error> {
                            let value = set.into_iter().try_fold(0,|mut acc,key| match key.to_string().as_str() {
                                #flag_matching_from_hashset
                                _ => Err(bit_fields::NonZeroTryFromFlagSetAndFieldMapError::MissingFlag)
                            })?;

                            let value = map.into_iter().try_fold(value,|mut acc,(k,v)|match k.to_string().as_str() {
                                #field_matching_from_hashmap
                                _ => Err(bit_fields::NonZeroTryFromFlagSetAndFieldMapError::UnknownRange)
                            })?;

                            #data_type::new(value).map(Self).ok_or(bit_fields::NonZeroTryFromFlagSetAndFieldMapError::Zero)
                        }
                    }
                }
            } else {
        ...
        let binary_ops = if data_type.is_nonzero() {
            quote! {
                impl #struct_name {
                    /// Carries out the [`std::ops::BitAnd`] operation returning `None` if it would
                    /// result in a zero value.
                    pub fn checked_bitand(self, rhs: Self) -> Option<Self> {
                        #data_type::new(self.0.get() & rhs.0.get()).map(Self)
                    }
                    /// Carries out the [`std::ops::Not`] operation returning `None` if it would
                    /// result in a zero value.
                    pub fn checked_not(self) -> Option<Self> {
                        #data_type::new(!self.0.get()).map(Self)
                    }
                }
            }
        } else {
        ...
        let equal_cmp = if data_type.is_nonzero() {
            quote! { (self.0.get() & mask) == (other.0.get() & mask) }
        } else {
            quote! { (self.0 & mask) == (other.0 & mask) }
        };

...
pub(crate) struct BitFieldMembersParser<'a> {
    ...
    error: bool,
    /// Skip attribute flag, indicating the next member should be skipped on serialization and deserialization.
    skip: bool,
...
pub(crate) enum BitFieldMembersParserIterError {
    ...
    #[error("Unknown attribute supplied on member. Supported attributes are `#[skip]` and `#[doc=\"..\"]`.")]
    UnknownAttribute,
    ...
    #[error("Cannot define member on bit range, as bit range does not exist. Bit ranges need to be defined before members which reference them.")]
    ...
    /// inconsistent). Thus this case would enable the same data to produce to bit fields of
    /// different values.
    #[error("Members which are present in the conversion to/from sets of flags and maps of fields cannot have overlapping bits as this leads to undefined behavior in as the value of the bit field would depend on the ordering of keys in a `HashMap` and/or `HashSet` (which is inconsistent). Thus this case would enable the same data to produce to bit fields of different values.")]
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            ...
            match self.iter.as_slice() {
                // Bit reference range: [ident, punct, ident, group [ literal, punct, punct, literal ], punct, ..] e.g. "see2: sse[0..2],"
